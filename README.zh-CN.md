...
启动时先播放短动画，随后进入单一会话界面。空会话保留动画标记但不再显示产品名称，并在动画下直接展示项目、分支、token 用量、上下文压力、当前模型和插件状态。一旦出现记录，默认折叠的历史仍会保留一行带高亮的 URI 状态栏；点击状态栏、按 `F4` 或运行 `:status` 可打开项目、会话和用量详情。常驻快捷提示行已移除。打开输入浮窗时，所有界面动画都会暂停，终端真实光标也会定位到文本插入点，使输入法候选窗保持稳定且定位准确。
