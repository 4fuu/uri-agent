...
`Shift+Enter` 插入换行，`Esc` 关闭输入浮窗并保留草稿，`:` 打开命令面板，`Tab` 补全匹配的命令，`F1` 显示当前生效的快捷键与命令参考。面板会对命令名、别名和描述进行模糊搜索，同时保持默认列表简洁。搜索内容只用于筛选命令；需要设置值的命令会打开选择面板或独立输入浮窗。`:new` 会直接切换到新的欢迎界面，不会重启终端界面。
