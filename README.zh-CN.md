...
启动时先播放短动画，随后进入单一会话界面。空会话保留原本居中的动画标记，下方只显示工作目录和当前 Provider/模型，再在局部居中的区域显示输入、命令和帮助的按键提示。一旦出现记录，默认折叠的历史仍会保留一行带高亮的 URI 状态栏；点击状态栏、按 `F4` 或运行 `:status` 可打开项目、会话和用量详情。打开输入浮窗时，所有界面动画都会暂停，终端真实光标也会定位到文本插入点，使输入法候选窗保持稳定且定位准确。
