...
- **ACP 编辑器集成：**`uri-agent --acpv1` 通过 stdio 提供稳定的 ACP v1 服务；由 ACP 创建的持久化会话之后可以在普通 TUI 中重新打开。
...
先在 TUI 中配置认证和默认模型，再将 ACP 客户端配置为启动：

```text
uri-agent --acpv1
```

客户端需要为每个会话提供绝对项目目录。ACP 创建的会话使用普通的持久化会话存储；客户端释放会话后，可在 TUI 中重新打开。支持的内容、MCP 服务器、生命周期操作和所有权约束见英文文档 [ACP v1](docs/acp.md)。
