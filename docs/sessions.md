...
Semantic conversation indexes are disposable sidecar caches under the platform
cache directory at `uri-agent/retrieval/v1/`. The `context` index is keyed by
session ID; the `sessions` index is keyed by the archive database path and
covers every project in that archive. Each manifest pins the extractor,
...
Rebuilds use a cross-process lock, create a complete replacement separately,
and activate it by rename, so searches never open a partially written
collection. Removing these directories loses no session data.
...
index. `exec("sessions://index", "")` starts a managed full rebuild of the
all-session semantic sidecar; `mode=semantic` performs vector ranking and
`mode=hybrid` fuses vector and Jieba BM25 ranks. Both ranked modes preserve
scope, working-directory, record-type, offset, and limit filtering and refuse
a missing or stale index. `@@` completion remains project-scoped. Archive
reads and indexing never resume or modify a session; see
...
- exact history search remains the default and uses record-ID pagination. `exec("context://history/index", "")` builds a managed semantic sidecar for the current session; user-statement and per-window searches then accept `mode=semantic` or `mode=hybrid` with relevance-order `offset` pagination. Ranked reads refuse a missing or stale index.
