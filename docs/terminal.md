...
Clicks and drags normally go to the terminal application. Hold `Shift` while dragging to select rendered text, or `Shift`-double-click to select a Unicode word, then use `Ctrl+C`, `Ctrl+Shift+C`, or right-click to copy through OSC52. On macOS, `Cmd+C` also works when the terminal forwards the modifier.

User prompts, assistant responses, blank conversation space (including the virtual tail), and read-only floats support direct drag selection and Unicode word selection by double-click. Hold `Shift` for either gesture on reasoning and tool blocks so ordinary clicks remain available for folding and opening. A conversation selection anchors to its blocks: it survives scrolling, streaming appends, and lazy history loads, so a drag can start in one viewport and extend after scrolling, and copying includes rows that are off screen. A press without a drag never starts a selection, so text auto-scrolling under a stationary cursor cannot create one. The selection is cleared when the transcript rewraps (for example on resize), when a block it touches is rewritten, collapses, or disappears, or when the session changes. On other surfaces, a selection follows its text while the surface scrolls and is cleared as soon as either end leaves the visible area. On URI Agent surfaces, a copy shortcut copies the active selection, `Esc` clears it, and any other shortcut clears it before continuing through normal key routing. From the conversation, `Ctrl+X` copies that selection or falls back to the latest nonempty assistant response; the macOS key style also accepts `Cmd+X`. In an open reasoning, tool, or process document, `c` copies the complete document rather than only the visible viewport. Terminal restoration, selection, and OSC52 copy remain active on normal and error exits.
