...
User prompts use a subtle teal background distinct from the footer, while assistant responses remain on the conversation background. Both use the full transcript width without decorative prefixes or selection rails, so copied text contains only message content. Assistant responses render as Markdown documents, including highlighted headings at every level, emphasis, links, lists, quotes, code blocks, and responsive tables. User prompts and assistant responses never fold. The first reasoning, tool, or assistant block after a prompt is separated from it by one blank row. Intermediate text remains adjacent to surrounding process blocks, while the final assistant response is separated from preceding reasoning or tools by one blank row once the turn finishes. Live reasoning expands by default, folds when the model advances to text or a tool call, and stays folded when the user folds it while deltas are still streaming. Completed reasoning and tool calls collapse to compact semantic summaries; tool rows describe the routed URI, command, patch targets, and result instead of exposing raw argument JSON. Process blocks align their semantic icons and indented details to the same transcript edges, and retain their selected-row background. Select a reasoning or tool block with the arrow keys or mouse and press `Enter` to expand or fold it. A single click performs the same action after the double-click interval; press `o` or double-click to open the full document immediately. The full-document float closes with `Esc` or a click outside its frame; outside clicks do not close other floats. Reasoning remains in the conversation instead of moving to a separate mode.
...
| Copy | `Ctrl+C` or right-click copies a selection; `Ctrl+Shift+C` copies the selection or visible surface; `Cmd+C` is accepted when the terminal forwards it |
