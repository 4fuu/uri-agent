...
Startup may show a short splash. An empty conversation then shows the project, active model and thinking effort, and compose, command, and help hints; if no model is configured, it prompts for `:login`. When networking is enabled and GitHub has a newer URI Agent release, the bottom of this welcome view shows its version and the releases address. The asynchronous check runs once per process, never blocks startup, and ignores network failures; `--offline` disables it. `:new` returns directly to this welcome view and reuses the completed check.

After the first record, a compact footer shows the active model, effort, and context usage. While background tasks are pending or running, it also shows their active count; click that task indicator or run `:tasks` to inspect them. The count disappears when no background task remains active. The context meter animates during live activity, stays static when idle, and uses provider-reported usage when available. `≈` appears only when an idle conversation remains on an estimate or an API baseline plus estimated trailing messages; live estimates omit it. `?` means a summary checkpoint invalidated the previous baseline and no new provider usage has arrived. While model output streams, the activity row appends a smoothed `tok/s` estimate after its animation. The estimate divides the visible words, CJK characters, and punctuation received since the response's first output by the elapsed time, calibrated against provider output usage, so pauses decay the value instead of clearing it; the estimate freezes while tools run. Once the complete turn is ready, the model and effort footer instead appends the visible-aligned provider output tokens divided by the summed first-output-to-completion time of its model responses. Hidden reasoning tokens are excluded unless reasoning text was visible; missing usage or timing suppresses the rate rather than showing a partial average. Activity and fixed statuses appear above the footer, while transient notifications overlay the current view and expire independently. Click elsewhere on the footer, press `F4`, or run `:status` for project, session, usage, cumulative model run time and average output rate, the active rollover or summary strategy, the per-session diagnostic log path, and extension details.
...
| `:compact` | Run the active context-checkpoint strategy immediately |
| `:context-strategy [rollover|summary]` | Toggle the active session strategy, or select one explicitly |
...
The `:tasks` panel follows task lifecycle changes while it is open, keeps the
current selection stable, and exposes cancellation for active work.
