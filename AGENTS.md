...
- Discover Skills once at startup from the documented project and user roots. Give every discovered Skill its own `<normalized-name>-skill` protocol. `://help` returns its `SKILL.md` plus the real Skill directory; resource routes must not escape that directory. Never compile or copy one developer environment's discovered Skill list into the product.
